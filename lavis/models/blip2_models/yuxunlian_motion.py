...
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def __init__(
        ...
        self.Qformer, self.query_tokens = self.init_motion_Qformer(
            8, 1024,
        )
    ...
    def generate(
        ...
        print(query_output.last_hidden_state[:,:query_tokens.size(1),:].shape,end='*')
        inputs_llm = self.llm_proj(query_output.last_hidden_state[:,:query_tokens.size(1),:])
        print(inputs_llm.shape,end='*')
        if is_video:
            inputs_llm = inputs_llm.reshape(B, -1, inputs_llm.shape[-1])
        atts_llm = torch.ones(inputs_llm.size()[:-1], dtype=torch.long).to(image.device)
    ...
    def from_config(cls, cfg):
        ...
        # model.query_tokens=torch.load('qformer_query_token_pretrain.pth')
        # model.Qformer.load_state_dict(torch.load('qformer_pretrain.pth'))


        #model.load_checkpoint_from_config(cfg)
        model.load_checkpoint(url_or_filename='lavis/output/humanml3d_caption/blip2_vicuna_instruct_vicuna7b/train/b48_e20_lr1e-05_wd0.05_q32_f80_fb40_freezevit/checkpoint_best.pth')
        return model