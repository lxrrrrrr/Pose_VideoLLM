...
import transformers
from lavis.models.blip2_models.motion_vq import loadVQModel
from lavis.common.registry import registry
from lavis.models.blip2_models.blip2 import Blip2Base, disabled_train, memory_bank_compress

@registry.register_model("blip2_llama3_instruct_malmm")
class Blip2LLAMA3Instruct_MALMM(Blip2Base):
    PRETRAINED_MODEL_CONFIG_DICT = {
        "Meta-Llama-3-8B-Instruct": "configs/models/blip2/blip2_instruct_Meta-Llama-3-8B-Instruct.yaml",
    }
    ...
    def __init__(
        ...
        ##########################################################################
        self.Qformer_motion, self.query_tokens_motion = self.init_motion_Qformer(
            8, 1024,
        )
        self.vq_encoder, self.quantizer = loadVQModel()
        for name, param in self.vq_encoder.named_parameters():
            param.requires_grad = False
        if not qformer_text_input:
            ...
            self.Qformer_motion.resize_token_embeddings(len(self.tokenizer))
        self.Qformer_motion.cls = None
        #########################################################################
        self.Qformer, self.query_tokens = self.init_Qformer(
            num_query_token,1408, memory_bank_length=memory_bank_length, num_frames=num_frames,
        )
        ...
        self.llm_proj_motion = nn.Linear(
            self.Qformer_motion.config.hidden_size, self.llm_model.config.hidden_size
        )
    ...
    def forward(self, samples):
        ...
        if self.qformer_text_input:
            ...
            if self.use_memory_bank:
                ...
                ###################################################################
                query_tokens_motion = self.query_tokens_motion.expand(B, -1, -1)
                query_atts_motion = torch.ones(query_tokens_motion.size()[:-1], dtype=torch.long).to(image.device)
                text_Qformer_motion = self.tokenizer(
                    samples["text_input"], # [B]
                    padding='longest',
                    truncation=True,
                    max_length=self.max_txt_len,
                ...
                Qformer_atts_motion = torch.cat([query_atts_motion, text_Qformer_motion.attention_mask], dim=1)
                keypoints_embeds = self.vq_encoder(keypoints[..., :-4]) #24 80 263->24 20 1024
                keypoints_atts = torch.ones(keypoints_embeds.size()[:-1], dtype=torch.long).to(image.device)
                query_output_motion = self.Qformer_motion.bert(
                    text_Qformer_motion.input_ids,
                    attention_mask=Qformer_atts_motion,
                    query_embeds=query_tokens_motion,
                    encoder_hidden_states=keypoints_embeds,
                    encoder_attention_mask=keypoints_atts,
                    return_dict=True,
                )
                ###################################################################
            else:
                ...
                image_embeds = self.vq_encoder(image[..., :-4]) #24 80 263->24 20 1024
        ...
        inputs_llm_keypoints = self.llm_proj_motion(query_output_motion.last_hidden_state[:,:query_tokens_motion.size(1),:])
        inputs_llm=torch.cat((inputs_llm,inputs_llm_keypoints),dim=1)
        if is_video:
    ...
    def from_config(cls, cfg):
        ...
        model.query_tokens=torch.load('pretrain_model/msvd_qa/qformer_query_token_pretrain.pth')
        model.Qformer.load_state_dict(torch.load('pretrain_model/msvd_qa/qformer_pretrain.pth'))
        model.llm_proj.load_state_dict(torch.load('pretrain_model/msvd_qa/llm_proj.pth'))

        model.query_tokens_motion=torch.load('pretrain_model/motion_token_8/qformer_motion_query_token_pretrain.pth')
        model.Qformer_motion.load_state_dict(torch.load('pretrain_model/motion_token_8/qformer_motion_pretrain.pth'))

        #model.load_checkpoint_from_config(cfg)
        #model.load_checkpoint(url_or_filename='lavis/output/humanml3d_caption/blip2_vicuna_instruct_vicuna7b/train/b48_e30_lr1e-05_wd0.05_q32_f80_fb40_freezevit/checkpoint_best.pth')
        return model